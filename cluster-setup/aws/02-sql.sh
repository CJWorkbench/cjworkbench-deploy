...
db_security_group="$(aws rds describe-db-instances --db-instance-identifier=cjworkbench --query DBInstances[0].VpcSecurityGroups[0].VpcSecurityGroupId --output text)"
cluster_security_group="$(aws eks describe-cluster --name=workbench --query cluster.resourcesVpcConfig.clusterSecurityGroupId --output text)"
